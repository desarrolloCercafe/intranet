...
@section('content')
    ...
    <title>Solicitar Insumos y Servicios | Cercafe</title>
    <script type="text/javascript"> 
        ...
        {   
            ...
            @foreach($productos as $producto)   
                var ref_insumo_servicio = <?php echo json_encode($producto->ref_insumo); ?>;
                var nombre_insumo_servicio = <?php echo json_encode($producto->nombre_insumo); ?>;
                if (granja_1 =='') {
                    swal({ 
                        title:"Granja Vacía",
                        text:'',
                ...
                else
                {
                    ...
                    {
                        if (text == ref_insumo_servicio)
                        {
                            $("#generar").show();
                            var id = <?php echo json_encode($producto->id); ?>;
                            var sptext = text.split();
                            var newtr = '<tr class="item" data-id="' + id + '">';
                            newtr = newtr + '<td class="iProduct">' + ref_insumo_servicio + '</td>';
                            newtr = newtr + '<td class="iProduct">' + nombre_insumo_servicio + '</td>';
    ...
    <div class="panel panel-default">
            <div class="panel-heading" id="titulo">
                <h4 style="font-size: 30px;"><i class="fa fa-list-alt" aria-hidden="true"></i> Nuevo Pedido de Insumos</h4>
            </div>
            <br>
            <div class="container-fluid">
                {!! Form::open(['class' => 'form-inline']) !!}
                    ...
                    <div class="form-group">
                        <button type="button" class="btn btn-success" id="agregar_medicamento" data-toggle="modal" data-target="#myModal"><span class="fa fa-plus"></span> Agregar Insumo</button>
                    </div>
                    <script type="text/javascript">
                        function enviarProductos()
                        {
                            ...
                            if (cont == 0)
                            {
                                ...
                                $.ajax({
                                    ...
                                    url: "http://201.236.212.130:82/intranetcercafe/public/admin/pedidoInsumosServicios",
                                    dataType: 'json',
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h1 class="modal-title" style="color: red;"><i class="fa fa-medkit fa-2x" aria-hidden="true""></i> Buscar Insumo</h1>
                </div>
                <div class="modal-body">
                    <label for="" class="control-label"><h4>Digite el nombre o codigo del Insumo en el siguiente campo:</h4></label>
                    <select class="form-control" id="pro_id" name="pro_id" style="width: 100% !important;">
                        <option>Seleccione...</option>
                        @foreach($productos as $producto)
                            @if($producto->disable != 1)
                                <option value="{{$producto->ref_insumo}}">{{$producto->nombre_insumo}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
