...
newMomentProto.format = function() {
	...
	if (this._ambigZone) {
		return oldMomentFormat(this, 'YYYY-MM-DD[T]HH:mm:ss A');
	}
...
var DayTableMixin = FC.DayTableMixin = {
	...
	renderHeadDateCellHtml: function(date, colspan, otherAttrs) {
		...
		return '' +
            '<th  style="color: #FFFFFF;" class="' + classNames.join(' ') + '"' +
				(this.rowCnt === 1 ?
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
				...
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					(isLabeled ?
						'<span>' + // for matchCellWidths
							htmlEscape(slotDate.format('hh:mm A')) +
						'</span>' :
						''
						) +
				'</td>';
			...
			html +=
				'<tr data-time="' + slotDate.format('hh:mm A') + '"' +
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (view.isMultiDayEvent(event)) { // if the event appears to span more than one day...
			...
			if (seg.isStart || seg.isEnd) {
				...
				startTimeText = this.getEventTimeText(seg, 'LT'); // displayEnd=false
			}
		} else {
			// Display the normal time text for the *event's* times
			timeText = this.getEventTimeText(event);
			fullTimeText = this.getEventTimeText(event, 'LT');
			startTimeText = this.getEventTimeText(event, 'LT'); // displayEnd=false
		}
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	buildGotoAnchorHtml: function(gotoOptions, attrs, innerHtml) {
		...
		if (!forceOff && this.opt('navLinks')) {
			return '<a style="color: #FFFFFF;"' + attrs +
	...
	initHiddenDays: function() {
		var hiddenDays = this.opt('hiddenDays') || [0]; // array of day-of-week indices that are hidden
...
var ListViewGrid = Grid.extend({
	...
	dayHeaderHtml: function(dayDate) {
		...
		return '<tr class="fc-list-heading" data-date="' + dayDate.format('YYYY-MM-DD') + '">' +
			'<td style="background: #DF0101;" class="' + view.widgetHeaderClass + '" colspan="3">' +
				(mainFormat ?
					view.buildGotoAnchorHtml(
						dayDate,
