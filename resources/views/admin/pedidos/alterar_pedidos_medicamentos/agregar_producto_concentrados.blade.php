...
@include('flash::message')
    <title>Agregar Concentrados | Cercafe</title>
    <script type="text/javascript"> 
        ...
        {
            @foreach($pr_db as $p)
                var ref_concentrado = <?php echo json_encode($p["codigo"]) ?>;
                var nombre_concentrado = <?php echo json_encode($p["descripcion"]) ?>;
                var kilos = <?php echo json_encode($p["kilos"]) ?>;
                var bultos = <?php echo json_encode($p["bultos"]) ?>;
                var unidad = <?php echo json_encode($p["unidad_medida"]) ?>;
                ...
                if (unidad == '1') 
                {
                	newtr = newtr + '<td class="iProduct"><input type="number" class="form-control" id="cant" name="ListaPro" value="'+ bultos +'"/></td>';
                	newtr = newtr + '<td class="iProduct"><input type="hidden" class="form-control" id="kilos_granel" name="ListaPro" readonly value="0"/></td>';
                	newtr = newtr + '<td class="iProduct" id="medida">bultos</td>';
                }
                else
                {
                	newtr = newtr + '<td class="iProduct"><input type="hidden" class="form-control" id="cant" name="ListaPro" readonly value="0"/></td>';
                    newtr = newtr + '<td class="iProduct"><input type="number" class="form-control" id="kilos_granel" name="ListaPro" value="'+ kilos +'"/></td>';
                	newtr = newtr + '<td class="iProduct" id="medida">kilos</td>';
                }
    ...
    <div class="panel panel-default">
        <div class="panel-heading" id="titulo">
            <h4 style="font-size: 25px;"><i class="fa fa-list-alt" aria-hidden="true"></i> Modificar el pedido PCO{{$consecutivo}}</h4>
        </div>
        <br>
        <div class="container-fluid">
            {!! Form::open(['class' => 'form-inline']) !!}
                ...
                <div class="form-group">
                    <input type="hidden" class="form-control" value="{{$fecha_estimada}}" id="fecha_estimada" placeholder="Fecha Estimada">
                </div>
                <div class="form-group">
                ...
                <script type="text/javascript">
                    function enviarProductos()
                    {
                        ...
                        $('#ProSelected').find('tr').each(function(i,el)
                        {
                            var granja = <?php echo json_encode($g); ?>;
                            var f_entrega = 'por verificar';
                            var $tds = $(this).find('td'); 
                            var codigo = <?php echo json_encode($consecutivo); ?>;
                            var concentrados = $tds.eq(1).text();
                            var unidad = $tds.eq(4).text();
                            ...
                            var fecha_estimada = $("#fecha_estimada").val(); 
                            var fecha_creacion =  <?php echo json_encode($fecha); ?>;
                        ...
                        if (cont == 0)
                        {
                            ...
                            $.ajax({
                                ...
                                url: "http://201.236.212.130:82/intranetcercafe/public/admin/pedidoConcentrados/show",
                                dataType: 'json',
