...
@section('content')
	...
	<title>Filtro Cia | Cercafe</title>
	<style type="text/css">
		a>strong{
			font-weight: 700;
		}
	...
	<div class="panel panel-default">
		...
		<div class="container-fluid col-xs-6 col-lg-12">
			 <div class="form-group">
                ...
                <a href="/intranetcercafe/public/admin/excelFiltradoPorPedidoCia/{{$f_ini}}/{{$f_fin}}/{{$grj}}" class="btn btn-success"><i>{!!Html::image('c.png','us',array('class' => 'imuser'))!!} Exportar</i></a>
			 </div>
		</div>
			...
			<table id="data_list" class="table table-bordered table-hover text-center" cellspacing="0" width="100%">
				...
				<tbody>
					@foreach($pedidos_db as $pedido_db)
		                <tr>
		                    <td>
		                        <button class="btn btn-link consecutivo" data-id="{{$pedido_db["consecutivo"]}}" data-toggle="modal" data-target="#myModal">PSE{{$pedido_db["consecutivo"]}}</button>
		                    </td>
		                    <td>{{ $pedido_db["granja"] }}</td>
		                    <td>{{ $pedido_db["fecha"] }}</td> 
		                    <td> 
                                <a href="/intranetcercafe/public/admin/excelPedidoCia/{{$pedido_db["consecutivo"]}}" class="btn btn-success"><i>{!!Html::image('c.png','us',array('class' => 'imuser'))!!} </i></a>
                                <a href="/intranetcercafe/public/admin/pdfPedidoCia/{{$pedido_db["consecutivo"]}}" class="btn btn-danger"><i>{!!Html::image('pdf.png','us',array('class' => 'imuser'))!!} </i></a>
                            </td>
                        </tr>
                    @endforeach
				</tbody>
