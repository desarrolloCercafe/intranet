...
@section('content')
	...
	<title>Destete Finalización | Cercafe</title> 
	<div class="panel panel-danger">
		<div class="panel-heading" id="titulo">
			<h4 style="font-size: 30px;" align="center"><i class="fa fa-plus-square" aria-hidden="true"></i><strong> Formulario Destete Finalización</strong></h4>
		</div>
		<div class="panel-body">
			{!!Form::open(['route'=> 'admin.desteteFinalizacion.store','id'=>'fechas', 'method'=>'POST', 'name'=>'formularioPrece'])!!}
				<div class="row">
					<input type="hidden" name="_token" class="form-control" value="{{ csrf_token() }}"> 
					<div class="form-group col-lg-6 col-xs-12">
						{!!Form::label('lote', 'Lote: ', ['class'=>'control-label'])!!}
						{!!Form::text('lote', null, ['class'=>'form-control', 'id'=>'loteId', 'placeholder' => '###'])!!}
					</div> 
					<div class="form-group col-lg-6 col-xs-12">
					...
					<div class="form-group col-lg-6 col-xs-12">
						...
						{!!Form::text('fecha_ingreso_granja',null, ['id'=>'fecha_ingreso','class'=>'form-control', 'required','onChange'=>'calcularFechas();', 'style' => 'cursor: pointer !important;', 'readonly', 'style' => 'cursor: pointer !important;'])!!}
					</div>
					<div class="form-group col-lg-6 col-xs-12">
						{!!Form::label('fecha_salida_granja', 'Fecha de Salida: ', ['class'=>'control-label'])!!}
						{!!Form::text('fecha_salida_granja',null, ['id'=>'fecha_salida','class'=>'form-control', 'required','onChange'=>'calcularFechas();', 'style' => 'cursor: pointer !important;', 'readonly', 'style' => 'cursor: pointer !important;'])!!}
					</div>
					...
					<div class="form-group col-lg-6 col-xs-12">
						{!!Form::label('meta_cerdos_entregar', 'Meta Cerdos a entregar: ', ['class'=>'control-label'])!!}
						{!!Form::number('meta_cerdos_entregar',null, ['class'=>'form-control', 'id'=>'metaCerdos', 'placeholder' => '...', 'required'])!!}
					</div>
					<div class="form-group col-lg-6 col-xs-12">
					...
					<div class="form-group col-lg-6 col-xs-12">
						{!!Form::label('dias_granja', 'Dias: ', ['class'=>'control-label'])!!}
						<i class="advertencia fa fa-exclamation-circle btn-sm pull-right" aria-hidden="true" data-toggle="tooltip" data-placement="left" title="Este campo generalmente es decimal, ingresar la Información con '.'(punto) y EVITANDO la ','(coma)."></i>
                    ...
                    {!!Form::hidden('ato_promedio',null, ['id'=>'ato_promedio','class'=>'form-control', 'readonly'])!!}
                     {!!Form::hidden('ato_promedio_dia',null, ['id'=>'ato_promedio_dia','class'=>'form-control', 'readonly'])!!}
                      {!!Form::hidden('conv',null, ['id'=>'conversion','class'=>'form-control', 'readonly'])!!}

					<div class="form-group col-lg-12 col-xs-12">
						<ul class="list-inline" align="center">
							<li> <a href="javascript:history.go(-1);" class="btn btn-danger">Cancelar</a></li>
							<li>
								{!!Form::submit('Registrar Información', array('class'=>'btn btn-success btn-md'))!!}
							</li>		
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		formulario.addEventListener('submit', e=> {
			...
			var meta_cerdos = document.getElementById('metaCerdos').value;
			...
			var aumento_promedio_inicial = document.getElementById('ato_promedio').value;
			var aumento_promedio_dia_inicial = document.getElementById('ato_promedio_dia').value;
			var conversion_fin = document.getElementById('conversion').value;
		...
		Swal({
		  ...
		  html: `
		        ...
		        <tr>
		         ...
		         <tr>
		          <th>Dias Permanencia</th>
		            <td>${Math.round(dias_permanencia)}</td>
		        </tr>
		         <tr>
		          <th>Edad Final</th>
		            <td>${edad_final}</td>
			...
			}).then((result) => 
			{
			  if (result.value) 
			  {
			  var url = "http://201.236.212.130:82/intranetcercafe/public/admin/desteteFinalizacion";
			   	$.ajax({
			   		type: "POST",
				   	url: url,                     
