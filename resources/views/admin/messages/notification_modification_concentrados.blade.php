...
<body>
	...
	<table id="customers">
		<tr>
			...
			<td><strong>Cantidad bultos: </strong></td>
			<td><strong>Cantidad kilos: </strong></td>
		</tr>
		@foreach($req as $pedido)
			<tr>
				@if($pedido["concentrado"] != null)
					...
					@if($pedido["unidad_medida"] == "kilos")
						<td class="datos"><strong>{{$pedido["kilos_granel"]}}</strong></td>
					@else
						<td class="datos"><strong>{{$pedido["kilos_bulto"]  * $pedido["cantidad"]}}</strong></td>
					@endif
				@endif
			</tr>
		@endforeach
